...
class PluginManagerGUI:
    ...
    def create_ui(self):
        ...
        ttk.Label(header, text="🔌 Plugin Manager", style="Title.TLabel").pack(anchor="w")
        ...
        # Refresh button
        ttk.Button(toolbar, text="🔄 Refresh", command=self.refresh_catalog).pack(side="left", padx=2)
    ...
    def populate_list(self):
        ...
        for plugin in self.catalog.get("plugins", []):
            ...
            # Format display text
            status = "✓" if self.is_plugin_installed(plugin) else "○"
            type_icon = {"DLL": "🪟", "Python": "🐍", "SO": "🐧"}.get(plugin.get("type"), "📦")
    ...
    def show_plugin_details(self, plugin):
        ...
        meta += f"👤 Author: {plugin.get('author', 'Unknown')}\n"
        meta += f"📁 Category: {plugin.get('category', 'General')}\n"
        
        if self.is_plugin_installed(plugin):
            meta += f"✓ Status: Installed\n"
        else:
            meta += f"○ Status: Not installed\n"
        ...
        if deps:
            ...
            for dep in deps:
                ...
                self.details_text.insert(tk.END, f"  • {dep_name} ({req})\n", "dep")
        ...
        if releases:
            self.details_text.insert(tk.END, f"\n📦 Available Versions:\n", "header")
            for rel in releases:
                self.details_text.insert(tk.END, f"  • {rel.get('version', 'Unknown')}\n")
    ...
    def update_action_buttons(self, plugin):
        ...
        if is_installed:
            self.install_btn.config(state="disabled", text="✓ Installed")
